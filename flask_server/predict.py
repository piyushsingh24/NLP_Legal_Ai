...
def simple_keyword_search(question, context):
    """
    Ultra-fast fallback: Finds the sentence in context with highest word overlap.
    """
...
def run_prediction(question_texts, context_text, model_path, n_best_size=5):
    """
    Hybrid Prediction: Gemini (Cloud) -> Keyword Search (Local Fallback)
    Rolled back to google.generativeai for stability.
    """
    ...
    for i, question in enumerate(question_texts):
        ...
        if GENAI_API_KEY:
            try:
                ...
                print(f"Gemini Unavailable ({e}). Switching to INSTANT fallback.", flush=True)
                gemini_success = False
        
        # 2. Fallback to Keyword Search (Instant)
        if not gemini_success:
            print("Using Keyword Search Fallback...", flush=True)
            answer_text = simple_keyword_search(question, context_text)
            print(f"Fallback Answer: {answer_text}", flush=True)
    ...
    for i, preds in enumerate(all_predictions):
        formatted_list = []
        for key in preds:
            formatted_list.append(preds[key])
...
def analyze_risks(context_text):
    """
    Analyzes the contract for legal loopholes and risks.
    Returns a dictionary with 'summary' and 'detailed' sections.
    """
    ...
    try:
        ...
        try:
            ...
            print("JSON Decode Error in Risk Analysis. Returning raw text.")
            return {
